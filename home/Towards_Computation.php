...
<body>
    <main id="top">
        ...
        <article>
            ...
            <p>Mechanisms that can control propagation of a signal depending on their state and that can change their state depending on another signal can be used for computers. = they can be built to take on any function.<br />
                Define and build functions that expand the spirits.<br />
                Spirits themselves resemble such functions. Bacteria, plants, brains, humans, computers, companies, countries, ecosystems.<br />
                They can choose to merge/become coherent.<br />
                Software is most flexible, can adapt to other substrates.</p>

            <p>Computing substrates are unified under hardware description languages, which define connections between given substrate-dependent logical units to form other desirable functions.<br />
                Implementation is expensive, so most hardware is general: provides basic functions that are controlled by series of instructions stored in memory.<br />
                Presumably, preinstalled programs (operating systems?) allow humans to write and execute new programs through keyboard, mouse, monitor.<br />
                Programming languages help humans state their wishes in terms of those instructions. All higher languages must eventually be compiled to machine code.</p>

            <hr />

            <h3 id="Hardware%20implementation">Hardware implementation</h3>
            <p>Signal propagation speeds: Flowing water ~ tens of m/s. Soundwaves in solids ~ thousands of m/s. Electromagnetic signals approach the speed of light ~300,000,000 m/s. Quantum entanglement allows instant transmission of information? Synapses/Axons?</p>

            <h4 id="Electronics">Electronics</h4>
            <p>They went outside and found that two groups of stuff attract each other but repel members of the same group and still nobody knows why. <i>charges</i>.<br />
                ...
                <br />
                When some materials touch some charged materials, the charge appears to spread into the new material als observed by a weaker attraction/repulsion to other charges.<br />
            ...
            <p>In a silicon crystal, electrons are can leave or join an atom core with similar energy. This means the crystal can be bombarded with two types of atoms in different places, one will provide an extra electron when it enters the crystal (n-type) and the other will want an electron from the silicon (p-type). Both <i>doped</i> regions will gain conductance while remaining electrically neutral. Pure silicon crystal can be used as a weak insulator/resistor.<br />
            ...
            <h3 id="Hardware%20description%20language">Hardware description language</h3>
...
<span class="p">}</span>
            ...
            <p>Use an <i>autoencoder</i> to reduce image size before training the unet. unet now predicts the noise in the <i>latents</i> (encoded images). use autoencoder's decoder to get high res image again.<br />
            ...
            <p>changing the world to my needs, this impossibly flexible thing appears that can take any shape that I can define within some performance limtations.<br />
                Run an LLM on it and it will speak to me.</p>
            <p>why care about implementation?<br />
                because it can be optimized and I want to build the next coolest thing, it should reflect the universe, not convention. How does it arise? its a general idea that can be differently implemented<br />
                implementations are unified by HDL<br />
                Software exists to reuse hardware.<br />
                Ugly software everywhere. Google owns me. How to build something better? Something that can be understood and that is mine? Dismante "Oh it's soo complicated, I have to buy into all this crap".<br />
                Build myself into the computer = spirit stream.<br />
                I answered "What is the spirit stream fundamentally?"<br />
                What is the spirit fundamentally? How does it want to instantiate itself? Build a machine that it can trust.</p>
            <p>Introspect to find that the spirits are somewhat substrate independent. that my body can be part of the environment and the environment can be part of my body.</p>
            <p>Layers of silicon, copper, insulation, doping = transistors + connections = gates<br />
                (HDL) Gates + Gates = von Neumann machine. ALU, memory, PC, registers<br />
                How to start the computer? Power delivery, BIOS?<br />
                Series of instructions come from compiled languages.<br />
                How to connect to other computers? Optic fibre cables, antennas, DNS<br />
                OS as the platform to build on.<br />
                Spirit stream interface</p>
            <p>How do neural networks actually running on the hardware? tinygrad<br />
                How does the brain connect into this framwork?</p>
            <h4 id="Electronics_1">Electronics</h4>
