    ...
    def parseBlocks(self, parent: etree.Element, blocks: list[str]) -> None:
        ...
        while blocks:
            if blocks[0].strip() == '':
                # Handle empty lines
                """
                purpose of this is to render empty blocks as empty paragraphs.
                there is a post processor that will replace those empty
                paragraphs with <br> elements which will finally render
                empty lines correctly. I suck at programming it seems.
                """
                self.blockprocessors["emptyline"].run(parent, blocks)
            else:
