...
function sunData(date, type) { // if type == 0 returns in minutes, if type == 1 returns as hh:mm

  const data = get_input_data(date);
  const jday = getJD(data.year, data.month, data.day);

  const rise = calcSunriseSet(1, jday, data.lat, data.lon, data.tz);
  const set = calcSunriseSet(0, jday, data.lat, data.lon, data.tz);

  if (type == 0) {
    return {rise: rise.timelocal, set: set.timelocal};
  } else if (type == 1) {
    return {rise: timeString(rise.timelocal,2), set: timeString(set.timelocal,2)}
  }

}

/**********************************************************/
/*              SUNRISE AND SET CALC DONE                 */
/**********************************************************/
